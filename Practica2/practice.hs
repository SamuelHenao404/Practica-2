...
import Data.List
import System.IO
import Control.Exception --Biblioteca para solucionar excepciones
import Control.DeepSeq --Biblioteca para forzar la evaluación de una expresión
...
-- Función para calcular el tiempo transcurrido en el parqueadero
tiempoEnParqueadero :: UTCTime -> UTCTime -> NominalDiffTime
tiempoEnParqueadero tiempoEntrada tiempoActual = diffUTCTime tiempoActual tiempoEntrada
...
cargarParqueadero = catch leerArchivo manejarExcepcion
    where
        leerArchivo = do
        ...
        manejarExcepcion :: IOException -> IO [Vehiculo]
        manejarExcepcion e = do
            putStrLn $ "Ocurrió un error al leer el archivo: " ++ show e
...
-- Funcion para listar los vehiculos en el parqueadero
...
cicloPrincipal parqueadero = do
    ...
    putStrLn "4. Lista de vehículos en el parqueadero"
    putStrLn "5. Salir"
    ...
    case opcion of
        ...
        "3" -> do
            ...
            case buscarVehiculo placaVehiculo parqueadero of
                Just vehiculo -> do
                    let tiempoTotal = tiempoEnParqueadero (entrada vehiculo) tiempoActual
